...
def _set_auth_parser(parser_auth: argparse.ArgumentParser) -> None:
    ...
    parser_auth_login.set_defaults(
        handler=Controller.auth_login_handler, parser=parser_auth_login
    )
...
def _set_task_pasrer(parser_task: argparse.ArgumentParser) -> None:
    ...
    parser_task_init.set_defaults(
        handler=Controller.task_init_handler, parser=parser_task_init
    )

    parser_task_create = parser_task_subparsers.add_parser("create")
    parser_task_create.set_defaults(
        handler=Controller.task_create_handler, parser=parser_task_create
    )
...
def _set_config_parser(parser_config: argparse.ArgumentParser) -> None:
    ...
    parser_config_use.set_defaults(
        handler=Controller.config_use_handler, parser=parser_config_use
    )
...
def _set_version_parser(parser_version: argparse.ArgumentParser) -> None:
    parser_version.set_defaults(
        handler=Controller.version_handler, parser=parser_version
    )
